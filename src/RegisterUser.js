...
// RegisterUser component for user registration
  ...
  return (
    <div>
      ...
      <div className="input-group mb-3">
        <span className="input-group-text btn btn-secondary" id="Username">
      ...
      <div className="input-group mb-3">
        <span className="input-group-text btn btn-secondary" id="Password">
...
function RegisterHandle(NewUserName, NewPassword, NewConfirmPassword) {
  if (NewUserName && NewPassword && NewConfirmPassword) {
    if (NewPassword === NewConfirmPassword) {
      fetch(`http://localhost:5000/Server/UserProfile/${NewUserName}`)
        .then((response) => response.json())
        .then((Data) => {
          if (!Data) {
            fetch("http://localhost:5000/Server/Register", {
              method: "POST",
              headers: {
